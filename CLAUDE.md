...
- `python generate_review_pdf.py` (repo root) prints the 'Categorization (merged)' sheet of `Review.ods` to `review/Reviewed papers categorization.pdf` via LibreOffice headless. Do not convert `Review.ods` to PDF directly — its stored print state breaks pages every ~2 rows; the script rebuilds a clean single-sheet copy first. Print settings (page size, column widths, font) are constants at the top of the script.
- `python generate_attribute_tables_pdf.py` (repo root) renders the three attribute tables (`review/{Node,Edge,Graph} attributes and performance measures.csv`) to PDFs of the same name, also via LibreOffice headless. The CSVs are the source of truth; regenerate the PDFs after editing them. Page size, margins, font, and the three column widths are constants at the top of the script. The description column must stay wide enough for the longest description, otherwise rows wrap.
